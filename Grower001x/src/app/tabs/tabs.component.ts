  ...
  implements OnInit, OnDestroy, AfterViewInit
{
  ...
  exportToExcel() {
    console.log(this.items02);

    //   Soho.excel.exportToCsv('GR- Ready to Process', this.items02);

    this.datagrid02?.exportToCsv('GR- Ready to Process');
  }
        ...
        for (const orderId of mtrIds) {
          this.userService.getUserContext().subscribe(() => {
            ...
            this.miService.execute(req2).subscribe((res2: IMIResponse) => {
              ...
              if (ORTY == 'SPT' && parseFloat(UDN1) > 0) {
                console.log('D00102 ' + ORTY + ' ' + UDN1 + ' ' + setqtyx);
                GRVAL2 = (parseFloat(UDN1) * parseFloat(RPQA)).toFixed(2);
              }
              ...
              if (
                ...
                console.log(
                  'D00102 ' + ORTY + ' ' + UDN1 + ' ' + parseFloat(SEQ2),
                );
                GRVAL2 = (parseFloat(UDN1) * parseFloat(SEQ2)).toFixed(2);
              }
  ...
  async onSelected01(args: any[], isSingleSelect?: boolean) {
    ...
    if (this.hasSelected) {
      ...
      console.log('qpuno ' + selected.F2PUNO);
      console.log('qpric ' + selected.APRRF);
      console.log('qudn1 ' + selected.IBUDN1);
      ...
      if (selected.IBUDN1 && selected.IBUDN1 !== '') {
        this.interactionService.sendMessage('aprice' + '~' + selected.IBUDN1);
      } else {
        this.interactionService.sendMessage('aprice' + '~' + selected.APRRF);
      }
